...
CONCAT(ROUND(
             SUM(CASE 
	              ...
	              WHEN promo_type = "BOGOF" THEN (base_price) * (`quantity_sold(after_promo)`/ 2) ELSE 0 
                  END )
...
CONCAT(ROUND(
      (
      SUM(`quantity_sold(after_promo)` - `quantity_sold(before_promo)`) / SUM(`quantity_sold(after_promo)`)
      )*100
,1),"%") AS ISU,
RANK() OVER (ORDER BY 
ROUND((SUM(`quantity_sold(after_promo)` - `quantity_sold(before_promo)`) / SUM(`quantity_sold(after_promo)`))*100,1)
DESC) AS ranking
